...
def load_dict(data):
    ...
    new_df = pd.DataFrame()
    new_df["type"] = df["type"]
    new_df["time_received_ns"] = df["time_received_ns"]
    new_df["sequence_num"] = df["sequence_num"]
    new_df["sampling_rate"] = df["sampling_rate"]
    new_df["sampler_address"] = df["sampler_address"]
    new_df["time_flow_start_ns"] = df["time_flow_start_ns"]
    new_df["time_flow_end_ns"] = df["time_flow_end_ns"]
    new_df["bytes"] = df["bytes"]
    new_df["packets"] = df["packets"]
    new_df["src_addr"] = df["src_addr"]
    new_df["dst_addr"] = df["dst_addr"]
    new_df["etype"] = df["etype"]
    new_df["proto"] = df["proto"]
    new_df["src_port"] = df["src_port"]
    new_df["dst_port"] = df["dst_port"]
